...
class _pick  extends  State<pickup>  {
 ...
 @override
  Widget build(BuildContext context) {
        i= widget.stRef.path;
        DocumentReference docRef =  FirebaseFirestore.instance.doc(widget.stRef.path);
        docRef.get().then(
  (DocumentSnapshot doc) {
    ...
    fontSize: 40,
            ...
            Row(
                  ...
                  children: [
                           if (_buttonVisible) Padding(
                      padding: const EdgeInsets.all(7.0),
                      child: Text('اختر مستلم الطالب لليوم :'
                      , style: TextStyle(
                     fontSize: 25,
                     ),),
                    ) else Container(
          ...
          child: ElevatedButton(
            ...
            onPressed: ()async {
              if(x==1){
                ...
                print(i);
                 DocumentReference docRef = await FirebaseFirestore.instance.doc(i);
                 docRef.update({"picked": "no" , "time": FieldValue.serverTimestamp()});
                  setState(() {
                      _buttonVisible = false;
                      _textToShow = "تم إبلاغ المدرسة بقدومك ";
                    });
                ...
                print(i);
                 DocumentReference docRef = await FirebaseFirestore.instance.doc(i);
                 docRef.update({"picked": "no" , "time": FieldValue.serverTimestamp(), "someone":"yes", "fullname":name, "nid":nid, "phone":phone,});
                 setState(() {
                      _buttonVisible = false;
