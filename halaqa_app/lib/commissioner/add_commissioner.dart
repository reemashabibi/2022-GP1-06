...
class AddCommissioner extends StatefulWidget {
  ...
  AddCommissioner(
      {Key? key, required this.schoolId, this.isUpdate = false, this.snapshot})
      : super(key: key);
...
class _AddCommissionerState extends State<AddCommissioner> {
  ...
  getStudent() async {
    ...
    for (var element in students) {
      //adding them in studentsName array
      DocumentSnapshot dc = await FirebaseFirestore.instance
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              Expanded(
                                  child: Text(
                                studentsName[i].toString(),
                                style: const TextStyle(
                                    fontSize: 16, color: Colors.black),
                              )),
                              Icon(
                                !checkStudentId.contains(students[i].id)
                                    ? Icons.check_box_outline_blank
                                    : Icons.check_box,
                                color: !checkStudentId.contains(students[i].id)
                                    ? Colors.black
                                    : Color.fromARGB(255, 116, 195, 255),
                              )
                            ],
                          ),
                        ),
                        ...
                        if (checkStudentId.isEmpty) {
                          ...
                          for (var element in checkStudentId) {
                            FirebaseFirestore.instance
                                ...
                                .doc(element)
                                .update({
                              "CommissionerId": FieldValue.arrayUnion([
                                FirebaseFirestore.instance
                                    .collection(
                                        'School/${widget.schoolId}/Commissioner/')
                                    .doc(widget.snapshot!.id)
                                ...
                                showDialog(
                                    ...
                                    builder: (context) {
                                      return const AlertDialog(
                                        content: Text(
                                          "تم إرسال بريدًا إلكترونيًا لإعادة تعيين كلمة المرور",
                                          style: TextStyle(
                                            color: Colors.green,
                                          ),
                                        ),
