...
$(document).on('submit', '.docForm', async function (e){
    ...
    if(!selectedClasses){
      document.getElementById('alertContainer').innerHTML = '<div style=" margin: 0 auto;"> <div class="alert error">  <input type="checkbox" id="alert1"/> <label class="close" title="close" for="alert1"> <i class="icon-remove"></i>  </label>  <p class="inner">يجب إختيار فصل واحد على الأقل </p> </div>';
      setTimeout(() => {
    ...
    if(classDocForm.Dname.value == ''){
      document.getElementById('alertContainer').innerHTML = '<div style=" margin: 0 auto;"> <div class="alert error">  <input type="checkbox" id="alert1"/> <label class="close" title="close" for="alert1"> <i class="icon-remove"></i>  </label>  <p class="inner">يجب تعيين اسم للمستند </p> </div>';
      setTimeout(() => {
    ...
    if(document.getElementById("file").files.length == 0){
      document.getElementById('alertContainer').innerHTML = '<div style=" margin: 0 auto;"> <div class="alert error">  <input type="checkbox" id="alert1"/> <label class="close" title="close" for="alert1"> <i class="icon-remove"></i>  </label>  <p class="inner">يجب ارفاق مستد </p> </div>';
      setTimeout(() => {
    ...
    if(format.test(file.name)){
      $('.loader').hide();
      document.getElementById('alertContainer').innerHTML = '<div style=" margin: 0 auto;"> <div class="alert error">  <input type="checkbox" id="alert1"/> <label class="close" title="close" for="alert1"> <i class="icon-remove"></i>  </label>  <p class="inner">@ يجب أن لا يحتوي اسم الملف على الرمز </p> </div>';
      setTimeout(() => {
...
for(var i=0; k<classesRef.length; i++){
  classesIds += classesRef[i].id+' ';
    ...
    querySnapshot.forEach(async(doc) => {
      ...
      try {
        var parentToken = pDoc.data().token;
         //send the notfication
         $.post("https://us-central1-halaqa-89b43.cloudfunctions.net/method/document",
         {
   ...
   .catch(error => {
    $(".loader").hide();
    console.log(error);
    document.getElementById('alertContainer').innerHTML = '<div style=" margin: 0 auto;"> <div class="alert error">  <input type="checkbox" id="alert1"/> <label class="close" title="close" for="alert1"> <i class="icon-remove"></i>  </label>  <p class="inner"> حصل خطأ يرجى المحاولة لاحقًا </p> </div>';
    setTimeout(() => {
...
$(document).on('click', '.changeDocSubmit',async function (e) {
  ...
  const DocForm = document.querySelector('.changeDocForm');
    ...
    querySnapshot.forEach(async(doc) => {
     //update the viewedLastDocument attribute to show the red bubble in the parent interface
      ...
      try {
        var parentToken = pDoc.data().token;
         //send the notfication
         $.post("https://us-central1-halaqa-89b43.cloudfunctions.net/method/documentUpdate",
      {
        token: pDoc.data().token,
   ...
   //end notification
      $('.loader').hide();
      document.getElementById('alertContainer').innerHTML = '<div style=" margin: 0 auto;"> <div class="alert success">  <input type="checkbox" id="alert2"/> <label class="close" title="close" for="alert2"> <i class="icon-remove"></i>  </label>  <p class="inner"> تم التغيير بنجاح</p> </div>';
      setTimeout(() => {
